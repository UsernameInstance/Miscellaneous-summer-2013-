...
void App::Run() { 
	...
	while(bRunning) {
		...
		if(pcModuleActive) {
			pcModuleActive->Loop();
			if(int(SDL_GetTicks()) > iLTime + 15) {
				iFPS = 1000/(SDL_GetTicks() - iLTime);
				iLTime = SDL_GetTicks();
				pcModuleActive->Render();
			}
			Text::Instance()->tString = Text::Instance()->ToString<int>(iFPS);
			Text::Instance()->RenderString(psSurfDisp, 0, 0, Text::Instance()->tString.c_str());
